...
void ExecuteCommand(std::vector<std::string> args)
{
	...
	{
		CONSOLE_ERROR("Unknown command \"%s\"", args[0].data());
	}
}
...
Game::dvar_s* getDvarDummy = nullptr;
...
BOOL RadiantMod_Init()
{
	...
	RegisterCommand("getdvar"s, [](std::vector < std::string > args) 
	{
		...
		if (args.size() == 1)
		{
			CONSOLE_ERROR("usage: getdvar <dvarName>");
			return;
		}

		std::string dvarType;
		getDvarDummy = Game::Dvar_FindVar(args[1].c_str());
		...
		if (getDvarDummy) 
		{
			...
			getDvarDummy = nullptr;
		} 

		else 
		{
			CONSOLE_ERROR("unkown dvar: \"%s\"", args[1].data());
		}
	});
	...
	RegisterCommand("setdvar"s, [](std::vector < std::string > args)
	{
		...
		if (args.size() <= 2)
		{
			CONSOLE_ERROR("usage: setdvar <dvarName> <value/s>");
			return;
		}
