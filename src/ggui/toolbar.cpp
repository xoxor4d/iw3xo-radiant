...
namespace ggui::toolbar
{
	...
	std::map<std::string, std::function<void()>> tbedit_elements;
	...
	void load_settings_ini()
	{
						...
						fs_homepath)
		{
			...
			for (const auto& element : tbedit_elements)
			{
				...
				{
					tbedit_ordered_list.emplace_back(
						tb_order_element_s(element.first, tb_element_id++, true, false));
				}
			}
		}
	}
	...
	void menu_toolbar_edit(ggui::imgui_context_menu& menu)
	{
		...
		if (ImGui::BeginTable("element_sort_table", 3,
			ImGuiTableFlags_SizingFixedFit | ImGuiTableFlags_NoPadOuterX | ImGuiTableFlags_BordersOuterV | ImGuiTableFlags_BordersOuterH))
		{
